...
* Read the changes to the specification.
...
* Read the changes to the tests.
  ...
  * These could be since the most recent previous commit.
  * These could be since a user-specified commit/tag.
* Read the file `acceptable_deviations.md`, if one exists.

Use the above as the most immediate context for the following.

* Perform the `GRID` analysis as detailed in the next section and give your observations.
* Do not give out an observation in case it is already recorded in `acceptable_deviations.md`.
